...
<head>
    <title>Produk Menu</title> <!-- Required meta tags -->
    <meta charset="utf-8">
    ...
    <div class="container">
        ...
        <div class="my-3">
            <div class="row">
                ...
                <div class="col-md-8">
                    <form class="form" method="get" action="{{ route('produk.search') }}">
                        <div class="row">
                            <div class="col-md-6">
        ...
        <div id="read" class="mt-3">
            <table class="table table-bordered">
                ...
                <tbody> @foreach($data as $ps) <tr>
                        <td>{{$ps->nama_produk}}</td>
                        <td>{{$ps->category_name}}</td>
                        <td>{{$ps->qty}}</td>
                        <td>{{$ps->harga}}</td>
                        <td> <a href="#"><i class="fa fa-pencil" aria-hidden="true" onClick="show({{$ps->produk_id}})"
                                    id="editCS"></i></a> | <a href="#"><i class="fa fa-minus-square " style="color:red"
                                    onClick="show_drop({{$ps->produk_id}})"></i></i></a> </td>
                    </tr> @endforeach </tbody>
            </table>
        </div>
    </div>
...
function show_drop(id) {
    $.get("produk/get/" + id, {
        ...
        $("#exampleModalLabel").html('Delete Produk')
        $("#page").html(data);
        $("#exampleModal").modal('show');
    });
